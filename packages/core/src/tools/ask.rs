...
const ASK_TOOL_DESCRIPTION: &str = r#"当遇到不确定或多种选择时，向用户提问以获取明确指示。
...
【Plan Mode 注意】
- 不要问 "我的计划准备好了吗？" — 用 ExitPlanMode 获取批准
- 不要问 "你对计划有反馈吗？" — 用户看不到计划直到 ExitPlanMode
- 不要在问题中引用 "the plan"

【最佳实践】
- 提供具体选项（不要只给问题描述）
- 提供推荐方案及理由
- 如果推荐特定选项，放在列表第一位并在 label 结尾加 "(推荐)" "#;
