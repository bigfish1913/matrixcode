...
use super::{
    ...
    ExecutorFactory, NodeExecutor, ToolExecutor, ToolExecutorConfig, ValidateExecutor,
    ...
    async fn chat(&self, _request: ChatRequest) -> anyhow::Result<ChatResponse> {
        Ok(ChatResponse {
            content: vec![ContentBlock::Text {
                text: "{\"result\": \"mock response\"}".to_string(),
            }],
...
async fn test_ai_executor_basic() {
    ...
    let result: Result<serde_json::Value> =
        NodeExecutor::execute(&executor, &node, &mut context).await;
    assert!(result.is_ok());
...
async fn test_ai_executor_with_template_params() {
    ...
    let params = HashMap::from([("input".to_string(), json!("{{user_input}}"))]);
...
async fn test_condition_executor_with_branches() {
    ...
    let result = NodeExecutor::execute(&executor, &node, &mut context)
        .await
        .unwrap();
...
async fn test_condition_executor_complex_conditions() {
    ...
    let result = NodeExecutor::execute(&executor, &node, &mut context)
        ...
        .unwrap();
    assert_eq!(
        result.get("matched_branch").unwrap(),
        &json!("complex_true")
    );
}
...
async fn test_composite_executor_first_success_all_fail() {
    ...
    let executors: Vec<std::sync::Arc<dyn NodeExecutor>> =
        vec![std::sync::Arc::new(ValidateExecutor::new())];
...
fn test_executor_factory_with_tool_names() {
    let factory =
        ExecutorFactory::new().with_tool_names(vec!["bash".to_string(), "read".to_string()]);
...
fn test_executor_factory_create_validate_executor_with_ai_no_provider() {
    ...
    let result: Result<std::sync::Arc<dyn NodeExecutor>> =
        factory.create_validate_executor_with_ai(config);
    match result {
        Err(e) => assert!(e.to_string().contains("Provider not configured")),
        Ok(_) => panic!("Should fail without provider"),
    }
...
fn create_task_node(id: &str, task: &str, params: HashMap<String, serde_json::Value>) -> NodeDef {
    NodeDef {
        id: id.to_string(),
        node_type: NodeType::Task,
        name: format!("{} Node", id),
