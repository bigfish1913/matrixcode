...
use super::transport::{Transport, TransportConfig, create_transport};
use super::types::*;
...
impl McpClient {
    ...
    pub async fn set_logging_level(&self, level: LogLevel) -> Result<()> {
        ...
        self.send_request("logging/setLevel", Some(serde_json::to_value(params)?))
            .await
    }
...
mod tests {
    ...
    fn test_client_builder() {
        let builder =
            McpClientBuilder::new("test").stdio("npx", vec!["-y".into(), "@playwright/mcp".into()]);
