...
use super::config::*;
use super::types::{MemoryCategory, MemoryEntry, AutoMemory};
...
fn parse_memory_response(json_text: &str, session_id: Option<&str>) -> Result<Vec<MemoryEntry>> {
    ...
    let entries = parsed.memories
        ...
        .filter_map(|item| {
            ...
            let mut entry = MemoryEntry::new(category, item.content, session_id.map(|s| s.to_string()));
...
fn deduplicate_entries(entries: Vec<MemoryEntry>) -> Vec<MemoryEntry> {
    ...
    entries
        ...
        .filter(|e| {
            ...
            if seen.iter().any(|s| AutoMemory::calculate_similarity(s, &content_lower) >= SIMILARITY_THRESHOLD) {
                false
            } else {
...
pub fn detect_memories_fallback(text: &str, session_id: Option<&str>) -> Vec<MemoryEntry> {
    ...
    let patterns: Vec<(MemoryCategory, Vec<&str>)> = vec![
        (MemoryCategory::Decision, vec![
            "最终决定", "决定采用", "我们决定", "选择使用", "采用方案",
            "定下来", "就定这个", "敲定", "拍板", "we decided", "final decision",
        ]),
        (MemoryCategory::Preference, vec![
            "我喜欢", "我偏好", "我习惯", "最常用", "一直用",
            "推荐", "建议使用", "首选", "i like", "i prefer",
        ]),
        (MemoryCategory::Solution, vec![
            "通过修改", "解决方案是", "搞定", "解决了",
            "修复成功", "改成", "优化了", "fixed by", "solved by",
        ]),
        (MemoryCategory::Finding, vec![
            "发现", "注意到", "原来", "找到问题",
            "定位到", "排查发现", "原因是", "found that", "discovered",
        ]),
        (MemoryCategory::Technical, vec![
            "技术栈是", "框架使用", "用的是", "基于",
            "tech stack", "using framework", "built with",
        ]),
        (MemoryCategory::Structure, vec![
            "入口文件是", "主文件位于", "项目结构是",
            "入口是", "目录是", "entry point", "main file",
        ]),
    ];
...
pub fn infer_category_from_content(content: &str) -> MemoryCategory {
    ...
    if lower.contains("决定") || lower.contains("选择") || lower.contains("采用") || lower.contains("decided") {
        return MemoryCategory::Decision;
    }
    if lower.contains("喜欢") || lower.contains("偏好") || lower.contains("习惯") || lower.contains("prefer") {
        return MemoryCategory::Preference;
    }
    if lower.contains("解决") || lower.contains("修复") || lower.contains("搞定") || lower.contains("fixed") {
        return MemoryCategory::Solution;
    }
    if lower.contains("发现") || lower.contains("原因") || lower.contains("原来") || lower.contains("found") {
        return MemoryCategory::Finding;
    }
    if lower.contains("技术") || lower.contains("框架") || lower.contains("库") || lower.contains("tech") {
        return MemoryCategory::Technical;
    }
    if lower.contains("文件") || lower.contains("目录") || lower.contains("入口") || lower.contains("file") {
