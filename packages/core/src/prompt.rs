...
在执行任何工具前，必须先完成以下思考步骤，并输出当前步骤标记：
...
- 可能的失败点在哪里？如何预防或应对？
- 是否需要回滚方案或备份？
【输出标记】⚠️ Step3: 风险识别 - [主要风险]

【第四步：执行验证】
- 每个步骤完成后，如何验证效果？
- 最终如何确认任务完全成功？
- 如果中途发现问题，如何调整方案？
【输出标记】✓ Step4: 执行验证 - [验证方法]

思考输出格式：
- 简单任务：内心快速思考即可，直接执行
- 中等任务：输出 Step 标记，简短说明
- 复杂任务：输出完整 Step 标记 + 方案对比表
...
- 使用你的理解能力判断：如果请求让你困惑或不确定，就问。不要依赖关键词匹配。"#;
