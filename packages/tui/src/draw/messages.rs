            ...
            match &msg.role {
                ...
                Role::Tool {
                    ...
                    if *is_pending {
                        ...
                        // Header: spinner + tool name + "executing"
                        lines.push(Line::from(vec![
                            Span::styled(
                        ...
                        // Show full input details from message content (stored as JSON)
                        if !msg.content.is_empty() {
                            if let Ok(input) = serde_json::from_str::<Value>(&msg.content) {
                                let detail_lines = extract_full_detail(name, &input);
                                for detail_line in detail_lines {
                                    // Highlight based on content type
                                    let detail_color = if detail_line.starts_with("$") {
                                        Color::Yellow  // Command: bright yellow
                                    } else if detail_line.starts_with("file:") || detail_line.starts_with("pattern:") || detail_line.starts_with("path:") || detail_line.starts_with("query:") || detail_line.starts_with("url:") {
                                        Color::Cyan    // Key fields: cyan
                                    } else {
                                        Color::Gray    // Other: gray
                                    };
                                    lines.push(Line::from(vec![
                                        Span::styled("    ", Style::default()),
                                        Span::styled(detail_line, Style::default().fg(detail_color)),
                                    ]));
                                }
                                
                                // Show more details for edit/write tools
                                if name == "edit" || name == "multi_edit" {
                                    // Show old_string preview (first line only)
                                    if let Some(old) = input.get("old_string").and_then(|v| v.as_str()) {
                                        if let Some(first_line) = old.lines().next() {
                                            let truncated = truncate(first_line, max_w.saturating_sub(12));
                                            lines.push(Line::styled(
                                                format!("    old: {}", truncated),
                                                Style::default().fg(Color::LightRed),
                                            ));
                                        }
                                    }
                                    // Show new_string preview (first line only)
                                    if let Some(new) = input.get("new_string").and_then(|v| v.as_str()) {
                                        if let Some(first_line) = new.lines().next() {
                                            let truncated = truncate(first_line, max_w.saturating_sub(12));
                                            lines.push(Line::styled(
                                                format!("    new: {}", truncated),
                                ...
                                if name == "write" {
                                    if let Some(content) = input.get("content").and_then(|v| v.as_str()) {
                                        ...
                                        if line_count > 0 {
                                            lines.push(Line::styled(
                                                format!("    content: {} lines", line_count),
                                                Style::default().fg(Color::Gray),
                                            ));
                                        }
                                    }
                        ...
                        continue; // Skip rest of tool rendering for pending state
                    }
