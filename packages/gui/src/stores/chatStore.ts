...
export interface LoopTask {
  id?: string;  // Optional ID for task cancellation
  message: string;
...
interface ChatState {
  ...
  // Set messages (for batch operations)
...
export const useChatStore = create<ChatState>((set, get) => ({
      ...
      const unlisten = await listen<AgentEvent>('agent-event', (event) => {
        ...
        switch (event_type) {
          ...
          case 'tool_use_input_delta': {
            // Streaming tool input - append delta string to existing tool input
            if (data && isToolUseInputData(data)) {
              set((s) => {
                const msgs = [...s.messages];
                const toolMsg = msgs.find(m => m.id === data.tool_use_input.id);
                if (toolMsg && toolMsg.role === 'tool') {
                  // Initialize as empty string if not set
                  if (typeof toolMsg.toolInput !== 'string') {
                    toolMsg.toolInput = '';
                  }
                  // Append delta string directly
                  (toolMsg.toolInput as string) += data.tool_use_input.delta;
                }
                return { messages: msgs };
              });
            }
          ...
          case 'tool_use_input_end': {
            // Tool input streaming complete - parse accumulated JSON string to object
            if (data && isToolUseInputData(data)) {
              set((s) => {
                const msgs = [...s.messages];
                const toolMsg = msgs.find(m => m.id === data.tool_use_input.id);
                if (toolMsg && toolMsg.role === 'tool' && typeof toolMsg.toolInput === 'string') {
                  try {
                    // Parse the accumulated JSON string into an object
                    const parsedInput = JSON.parse(toolMsg.toolInput as string);
                    toolMsg.toolInput = parsedInput;
                  } catch (parseError) {
                    // If JSON parsing fails, keep as string (malformed input)
                    console.warn('Failed to parse tool input JSON:', parseError);
                  }
                }
                return { messages: msgs };
              });
  ...
  stopLoopTask: async () => {
    ...
    if (task && task.isActive) {
      try {
        // Call backend to cancel the loop task
        await invoke('cancel_task', { taskId: task.id });
        set({ loopTask: null });
      } catch (e) {
        console.error('Failed to cancel loop task:', e);
  ...
  stopCronTask: async (id: number) => {
    try {
      // Call backend to cancel the cron task
      await invoke('cancel_task', { taskId: String(id) });
      const tasks = get().cronTasks;
      const updated = tasks.filter(t => t.id !== id);
      set({ cronTasks: updated });
    } catch (e) {
      console.error('Failed to cancel cron task:', e);
