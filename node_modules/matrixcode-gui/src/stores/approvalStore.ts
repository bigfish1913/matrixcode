...
interface ApprovalState {
  ...
  // Event listener cleanup (can be 'pending' during initialization)
  _unlisten: UnlistenFn | 'pending' | null;
...
export const useApprovalStore = create<ApprovalState>((set, get) => ({
  ...
  approve: async (id: string) => {
    ...
    try {
      ...
      throw e; // Re-throw so caller can show error feedback
    }
  },
  ...
  startListening: async () => {
    // Use a flag to prevent race condition - check and set atomically
    const currentUnlisten = get()._unlisten;
    if (currentUnlisten) return;

    // Set flag immediately to prevent concurrent calls
    set({ _unlisten: 'pending' });
    ...
    try {
      ...
      set({ _unlisten: null }); // Reset flag on failure
    }
  },
  ...
  removeApprovalRequest: (id: string) => {
    set((s) => {
      const remaining = s.pendingApprovals.filter(r => r.id !== id);
      return {
        pendingApprovals: remaining,
        currentApproval: remaining.length > 0
          ? remaining[0] // Use first remaining item
          : null,
      };
    });
  },
