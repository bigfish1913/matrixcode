...
export function WorkflowPanel({ workflowState, onToggle, onSelectNode, onChangeViewMode }: WorkflowPanelProps) {
  ...
  // Sync viewMode when prop changes
  useEffect(() => {
    if (workflowState.viewMode && workflowState.viewMode !== viewMode) {
      setViewMode(workflowState.viewMode);
    }
          ...
          >
            ...
            {node.status === 'failed' && node.error && (
              <div className="mt-1 text-xs text-red-600 truncate">
                错误: {node.error.length > 50 ? node.error.slice(0, 50) + '...' : node.error}
              </div>
            )}
          </button>
        ))}
