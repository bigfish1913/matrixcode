...
// Priority indicators with labels
const TODO_PRIORITY_CONFIG: Record<string, { color: string; label: string }> = {
  high: { color: 'text-red-500', label: '高优先级' },
  medium: { color: 'text-yellow-500', label: '中优先级' },
  low: { color: 'text-gray-400', label: '低优先级' },
};

// Single todo item
function TodoItemView({ todo }: { todo: TodoItem }) {
  const statusInfo = TODO_STATUS_ICONS[todo.status];
  const priorityInfo = todo.priority ? TODO_PRIORITY_CONFIG[todo.priority] : null;

  return (
    <div className="flex items-center gap-2 py-1 px-2 rounded">
      {/* Status icon */}
      <span className={`font-mono text-sm ${statusInfo.color}`} aria-label={`状态: ${todo.status}`}>
        {statusInfo.icon}
      </span>
