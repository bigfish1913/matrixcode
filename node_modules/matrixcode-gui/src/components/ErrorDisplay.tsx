...
export function ErrorDisplay({ error, onRetry, onDismiss, onCopy }: ErrorDisplayProps) {
  ...
  return (
    <div
      className={`rounded-lg border p-4 ${SEVERITY_BG[classification.severity]}`}
      role="alert"
      aria-live="assertive"
      aria-label={`错误: ${typeConfig.label} - ${classification.severity}`}
    >
      {/* Header */}
      <div className="flex items-start gap-3">
        <span className={`text-2xl ${typeConfig.color}`} aria-hidden="true">
          {typeConfig.icon}
        </span>
