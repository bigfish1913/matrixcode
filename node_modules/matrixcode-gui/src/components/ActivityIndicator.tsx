...
export function ActivityIndicator({ activity, detail, elapsedSeconds }: ActivityIndicatorProps) {
  ...
  return (
    <div
      className="flex items-center gap-2 px-4 py-2 bg-card border rounded-lg mb-2"
      role="status"
      aria-live="polite"
      aria-label={`活动: ${config.label}${detail ? `, ${detail}` : ''}${showElapsed ? `, ${formatElapsed(elapsedSeconds)}` : ''}`}
    >
      {/* Activity icon */}
      <span className={`${config.color} text-lg animate-pulse`} aria-hidden="true">
        {config.icon}
      </span>
